...
node {
    ...
    sh "cp /opt/omnissa/dux/pr_manifest.yml ${env.WORKSPACE}/pr_manifest.yml"
}
pipeline {
    agent any
    environment {
        UEM_PASSWORD = credentials('uem-pword') // Fetch the UEM password from Jenkins credentials
        API_TOKEN = credentials('pr_api_token') // Fetch the API token from Jenkins credentials
    }
    ...
    parameters {
        ...
        string(
                    name: 'manifestPath',
                    defaultValue: '/opt/omnissa/dux/pr_manifest.yml',
                    description: 'Path to the manifest file for PAC Reader')
    ...
    stages {
        ...
        stage('Check pr_manifest.yml Changes') {
            steps {
                script {
                    def manifestPath = params.manifestPath
                    def lastHashFile = '/var/lib/jenkins/last_pr_manifest_md5'
                    ...
                    // Calculate current hash of pr_manifest.yml
                    ...
                    """
                    if (!lastHashExists) {
                    echo "First time run: Pushing pr_manifest.yml to GitHub with comment 'created pr_manifest.yml'"
                    withCredentials([sshUserPrivateKey(credentialsId: CLUSTER_CREDS_GIT_CRED_REF, keyFileVariable: 'SSH_KEY')]) {
                        sh """
                            ...
                            git add ${configDir}/pr_manifest.yml
                            git commit -m 'created pr_manifest.yml'
                    ...
                    writeFile file: lastHashFile, text: currentHash
                        ...
                        echo "File has changed: Pushing pr_manifest.yml to GitHub with comment 'user edit'"
                        withCredentials([sshUserPrivateKey(credentialsId: CLUSTER_CREDS_GIT_CRED_REF, keyFileVariable: 'SSH_KEY')]) {
                            sh """
                                GIT_SSH_COMMAND='ssh -i ${SSH_KEY}' git clone -b ${branchName} ${CLUSTER_CREDS_REPO} repo
                                cp ${manifestPath} repo/${configDir}/
                        ...
                        writeFile file: lastHashFile, text: currentHash
                            ...
                            echo "No changes detected in pr_manifest.yml"
                        }
                    }
                }
            }
        }
       /*  stage('Pre-check Hosts in pr_manifest.yml') {
            steps {
                script {
                    def manifestPath = '/opt/omnissa/dux/pr_manifest.yml'
            ...
            steps {
                script {
                    ...
                    if (env.DUX_MAJOR_VERSION.toInteger() >= 3) {
                        echo "Dux version is 3 or higher. Running 'dux pacreader deploy -d'..."
                        command = 'dux pacreader deploy -d' 
                    } else {
        ...
        stage('Run Dux Deploy with UEM Password') {
            steps {
                script {
                    ...
                    try {
                        // Execute the `dux deploy` command
                        def deployOutput = sh(script: "${command} 2>&1", returnStdout: true).trim()
                        echo "dux deploy command output:\n${deployOutput}"

                        if (!deployOutput.contains("Deploy command has completed")) {
                            error "Dux Deploy for PAC Reader failed. Exiting pipeline."
                        }
        ...
        stage('Check Dux Status') {
            steps {
                script {
                    ...
                    if (env.DUX_MAJOR_VERSION.toInteger() >= 3) {
                        echo "Dux version is 3 or higher. Running 'dux pacreader status'..."
                        command = params.HOST_IP == 'All' ? 'dux pacreader status' : "dux pacreader status -p ${params.HOST_IP}"
                    } else {
                        error "Dux version is less than 3.0 . PAC Reader container is not supported."
                    }
...
def getHostIPs() {
    ...
    node {
        try {
            def manifestPath = "${env.WORKSPACE}/pr_manifest.yml"
            ...
            if (manifest.pac_reader?.hosts) {
                manifest.pac_reader.hosts.each { host ->
                    if (host.address) {
                        ips << host.address
                    }
                }
