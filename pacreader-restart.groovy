            ...
            steps {
                script {
                    ...
                    if (env.DUX_MAJOR_VERSION.toInteger() >= 3) {
                        echo "Dux version is 3 or higher. Running 'dux pacreader restart'..."
                        command = params.HOST_IP == 'All' ? 'dux pacreader restart -y' : "dux pacreader restart -y -p ${params.HOST_IP}"
                    } else {
                        error "Dux version is less than 3.0 . pacreader container is not supported."
                    }
                    try {
