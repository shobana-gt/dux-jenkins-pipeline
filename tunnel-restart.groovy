            ...
            steps {
                script {
                    ...
                    if (env.DUX_MAJOR_VERSION.toInteger() >= 3) {
                        echo "Dux version is 3 or higher. Running 'dux tunnel restart'..."
                        command = params.HOST_IP == 'All' ? 'dux tunnel restart -y' : "dux tunnel restart -y -p ${params.HOST_IP}"
                    } else {
                        echo "Dux version is less than 3. Running 'dux restart'..."
                        command = params.HOST_IP == 'All' ? 'dux restart -y' : "dux restart -y -p ${params.HOST_IP}"
                    }
                    try {
