...
pipeline {
    ...
    stages {
        ...
        stage('Prepare PAC Reader Config Directory') {
            steps {
                script {
                    echo "Preparing PAC Reader config files from cluser configuration..."
                    sh """
                        mkdir -p /opt/omnissa/dux/
                    """
                }
            }
        }
        stage('Copy Files to Target Directory') {
            steps {
                script {
                    echo "Copying files from repo/config/pacreader-config to /opt/omnissa/dux/ ..."
                    sh """
                        cp repo/config/pacreader-config/* /opt/omnissa/dux/
                    """
                }
            }
        }
        ...
        stage('Initialize PAC Reader Manifest') {
            steps {
                script {
                    ...
                    if (manifestExists) {
                        ...
                        if (majorVersion >= 3) {
                            echo "Dux version is 3.0 or higher. Running 'dux pacreader init' to create the manifest..."
                            sh "dux pacreader init"
                        } else {
                            error "Dux version is less than 3.0. PAC Reader Container is not supported. Exiting pipeline."
                        }
                    }
                }
            }
        }
        stage('Check pr_manifest.yml') {
            steps {
                script {
                    def manifestPath = "/opt/omnissa/dux/pr_manifest.yml"
                    if (fileExists(manifestPath)) {
                        echo "pr_manifest.yml already exists. Update the manifest if needed."
                    } else {
                        echo "pr_manifest.yml does not exist. Proceed with creating it if required."
                    }
                }
            }
        }
    ...
    post {
        success {
            echo "PAC Reader: initialization pipeline completed successfully."
        }
        failure {
            echo "PAC Reader: initialization pipeline failed."
        }
    }
}