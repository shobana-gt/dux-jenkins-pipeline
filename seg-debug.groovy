...
/*         stage('Prepare Workspace') {
        ...
        stage('Debug File Existence') {
            steps {
                script {
                    def fileExists = fileExists("${env.WORKSPACE}/seg_manifest.yml")
        ...
        stage('Run Dux Logs') {
            steps {
                script {
                    echo "Fetching SEG container logs for troubleshooting..."
                    def command = ""

                    if (env.DUX_MAJOR_VERSION.toInteger() >= 3) {
                        echo "Dux version is 3 or higher. Running 'dux seg logs'..."
                        command = params.HOST_IP == 'All' ? 'dux seg logs' : "dux seg logs -p ${params.HOST_IP}"
                    } else {
