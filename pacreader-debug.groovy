...
/*         stage('Prepare Workspace') {
        ...
        stage('Debug File Existence') {
            steps {
                script {
                    def fileExists = fileExists("${env.WORKSPACE}/pr_manifest.yml")
        ...
        stage('Run Dux Logs') {
            steps {
                script {
                    echo "Fetching PAC Reader container logs for troubleshooting..."
                    def command = ""

                    if (env.DUX_MAJOR_VERSION.toInteger() >= 3) {
                        echo "Dux version is 3 or higher. Running 'dux pacreader logs'..."
                        command = params.HOST_IP == 'All' ? 'dux pacreader logs' : "dux pacreader logs -p ${params.HOST_IP}"
                    } else {
